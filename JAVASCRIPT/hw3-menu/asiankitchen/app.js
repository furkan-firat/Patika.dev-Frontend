...
let itemContainer = document.querySelector(".section-center");
...
let AllFoods = itemContainer.innerHTML
...
buttons.forEach(button => {
  button.addEventListener('click', function() {
    ...
    // Find the corresponding menu item object based on its id
    const menuItems = menu.filter(item => item.category == itemId);
    

    // Display the menu item information in the output element
    if (menuItems.length === 0) {
        itemContainer.innerHTML = AllFoods
    }else{
      itemContainer.innerHTML = ""
